...
function computeNameScore(nameA, nameB) {
  ...
  let base = Math.max(jw, tsr);

  const tokensA = nameA.trim().split(/\s+/).filter(t => t.length > 0);
  const tokensB = nameB.trim().split(/\s+/).filter(t => t.length > 0);
  ...
  if (tokensA.length >= 2 && tokensB.length >= 2) {
    ...
    if (firstA === firstB && lastA !== lastB) {
      const surnameScore = jaroWinkler(lastA, lastB);
      if (surnameScore < 0.6) {
        base *= 0.65;
      } else if (surnameScore < 0.8) {
        base *= 0.8;
      }
    }
  }
